...
main(void)
{
	uint8_t sequence = 0, direction = CW, speed = 128; /* 0 - 255 */
	...
	while (1)
	{
		...
		for (uint8_t i = 0; i < (255 - speed) + 1; ++i)
		{
			for (uint8_t j = 0; j < 32; ++j)
			{
				...
				__builtin_avr_delay_cycles(20); // PWM high level period (50%)
				CLR(IN1);CLR(IN2);CLR(IN3);CLR(IN4);
				__builtin_avr_delay_cycles(20); // PWM low level period (50%)
			}
		}
	}
}
