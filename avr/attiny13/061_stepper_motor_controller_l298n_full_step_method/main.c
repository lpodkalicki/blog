 ...
 * ATtiny13/061
 * Trivial example of controlling a stepper motor with Full-Step method
 ...
 *             (RED)     (BLU)
 * _________________________________
 * Pattern for a Full-Step method:
 * +--------+---+---+---+---+
 * |  STEP  | A | B | C | D |
 * +--------+---+---+---+---+
 * |   1    | 1 | 1 | 0 | 0 |
...
main(void)
{
	...
	while (1)
	{
		...
		switch (sequence)
		{
		// Using pattern for Full-Step method
		//      [   A  ][   B   ][   C   ][   D   ]
		case 0: SET(IN1);SET(IN2);CLR(IN3);CLR(IN4);break;
		case 1: CLR(IN1);SET(IN2);SET(IN3);CLR(IN4);break;
		case 2: CLR(IN1);CLR(IN2);SET(IN3);SET(IN4);break;
		case 3: SET(IN1);CLR(IN2);CLR(IN3);SET(IN4);break;
		}
		_delay_ms(1); // 1khz
	}
}
